...
function buildMesProfs(){
  ...
  if(!carousel)return;
  var _STATUT_LBL={'etudiant':t('statut_etudiant'),'prof_ecole':t('statut_prof_ecoles'),'prof_college':t('statut_prof_clg'),'prof_universite':t('statut_chercheur'),'auto':t('statut_auto'),'autre':t('statut_autre')};
  ...
  if(enrolledProfs.length){
    ...
    enrolledProfs.forEach(function(ep){
      ...
      var subLine='<span style="color:var(--or);font-weight:600">'+t('espace_inscrit')+'</span>'+(statut?'<span style="color:var(--lite)"> · '+esc(statut)+'</span>':'');
  ...
  if(folIds.length){
    html+='<div style="font-size:11px;font-weight:700;color:var(--lite);text-transform:uppercase;letter-spacing:.07em;padding:16px 20px 8px">'+t('acc_profs_suivis')+'</div>';
    folIds.forEach(function(pid){
      ...
      var statut=fresh?(_STATUT_LBL[p.statut]||p.rl||t('reg_prof')):'';
...
function openEspaceEleve(pid){
  ...
  if(hdEl){
    hdEl.innerHTML='<div style="width:52px;height:52px;border-radius:50%;background:'+avBg+';display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;color:#fff;overflow:hidden;flex-shrink:0">'+avHtml+'</div>'
      ...
      +'<div style="font-size:13px;color:var(--or);font-weight:600;margin-top:2px">'+t('espace_inscrit')+'</div></div>';
  }
  // Show page
...
function openEnrollSheet(){
  ...
  sheet.innerHTML=
    ...
    +'<div style="font-size:19px;font-weight:800;color:var(--ink);letter-spacing:-.03em;margin-bottom:4px">'+t('enroll_title')+'</div>'
    +'<div style="font-size:13px;color:var(--lite);margin-bottom:22px">'+t('enroll_sub')+'</div>'
    ...
    +'<button id="_enrollBtn" onclick="submitEnrollSheet()" style="width:100%;background:var(--or);color:#fff;border:none;border-radius:16px;padding:15px;font-family:inherit;font-weight:700;font-size:16px;cursor:pointer;box-shadow:0 4px 14px rgba(255,107,43,.28);margin-top:4px">'+t('enroll_btn')+'</button>'
...
function getGreeting(){
  ...
  if(h>=12&&h<18)return t('greet_afternoon');
  if(h>=18&&h<22)return t('greet_evening');
...
function switchATab(s,el){
  ...
  if(s==='Rev'){loadRevenues();loadPayouts();loadStripeConnectStatus();}
...
function buildAccLists(){
  ...
  if(false&&isProf){
    ...
    profCoursHtml='<div style="padding:20px 20px 0">'
      ...
      +'<div style="font-size:13px;font-weight:800;color:var(--ink)">'+t('section_prochains')+'</div>'
      +(_pastCnt>0?'<div style="margin-left:auto;font-size:11px;color:var(--lite)">'+_pastCnt+' passé'+((_pastCnt>1)?'s':'')+'</div>':'')
      +'</div>';
    if(!myC.length){
      var _emptyMsg=_pastCnt>0?t('empty_no_cours_prof'):t('empty_first_cours');
      profCoursHtml+='<div style="background:var(--bg);border-radius:16px;padding:20px;text-align:center">'
        ...
        +'<button onclick="navTo(\'exp\')" style="background:var(--or);color:#fff;border:none;border-radius:50px;padding:10px 20px;font-family:inherit;font-weight:700;font-size:13px;cursor:pointer">'+t('btn_creer_cours')+'</button>'
        +'</div>';
    } else {
    ...
    profCoursHtml+='</div>'
      ...
      +'<div style="font-size:13px;font-weight:800;color:var(--ink)">'+t('section_mes_resa')+'</div>'
      +'</div>';
  }
  ...
  if(!_showRIds.length){lr.innerHTML+=isProf
    ?'<div style="padding:0 20px 20px;font-size:13px;color:var(--lite)">'+t('empty_no_resa_prof')+'</div>'
    ...
    +'</div>'
    +'<div style="font-size:16px;font-weight:700;color:var(--ink);margin-bottom:8px">'+t('empty_resa_title')+'</div>'
    +'<div style="font-size:14px;color:var(--lite);line-height:1.6;margin-bottom:20px">'+t('empty_resa_sub')+'</div>'
    +(_pastRIds.length?'<button onclick="switchATab(\'H\',g(\'aTabH\'))" style="background:var(--bg);color:var(--mid);border:1.5px solid var(--bdr);border-radius:50px;padding:10px 20px;font-family:inherit;font-weight:600;font-size:13px;cursor:pointer;margin-bottom:10px">'+t('empty_resa_histo')+' ('+_pastRIds.length+')</button><br>':'')
    +'<button onclick="navTo(\'exp\')" style="background:var(--or);color:#fff;border:none;border-radius:50px;padding:12px 24px;font-family:inherit;font-weight:700;font-size:14px;cursor:pointer;box-shadow:0 4px 14px rgba(255,107,43,.3)">'+t('empty_resa_explorer')+'</button>'
    +'</div>';}
  else{
...
function openR(id){haptic(4);
  ...
  if(_rBtn&&!_rBtn.disabled){_rBtn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" width="13" height="13" style="animation:cpSpin .6s linear infinite"><path d="M21 12a9 9 0 1 1-6.22-8.56"/></svg>';_rBtn.disabled=true;var _rBtnTid=setTimeout(function(){if(_rBtn){_rBtn.innerHTML=t('card_reserve');_rBtn.disabled=false;}},5000);_rBtn._resetTid=_rBtnTid;}
  ...
  if(_rBtn&&_rBtn._resetTid){clearTimeout(_rBtn._resetTid);_rBtn.innerHTML=t('card_reserve');_rBtn.disabled=false;}
  openM('bdR');
}
...
function _renderMpfExtraInfo(p){
  ...
  if(formations){
    rows+='<div class="mpf-section-lbl">'+t('prof_formations')+'</div>'
  ...
  if(experiences){
    rows+='<div class="mpf-section-lbl">'+t('prof_experiences')+'</div>'
      +'<div style="background:var(--wh);border-radius:16px;padding:14px;margin:0 16px 8px">'
...
function _buildBadges(p,pid){
  ...
  if(fol.has(pid))h+='<span class="prof-badge prof-badge-fol">'+icoFol+t('mp_suivi')+'</span>';
  return h;
}
...
function espRegenCode(){
  ...
  if(_espCurrentCode&&btn&&btn.dataset.confirm!=='1'){
    ...
    btn.textContent=t('esp_code_confirm');
    btn.style.background='rgba(239,68,68,.15)';
    btn.style.color='#EF4444';
    setTimeout(function(){
      if(btn.dataset.confirm==='1'){
        btn.dataset.confirm='';
        btn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" width="14" height="14"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg> '+t('esp_code_nouveau');
        btn.style.background='';btn.style.color='';
      }
    },3000);
    return;
  }
  if(btn){btn.dataset.confirm='';btn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" width="14" height="14"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg> '+t('esp_code_nouveau');btn.style.background='';btn.style.color='';}
  if(el)el.textContent='⋯';
...
function espShareCode(){
  if(!_espCurrentCode){toast(t('t_generate_first'),'');return;}
  var txt=t('esp_share_text').replace('{code}',_espCurrentCode);
  if(navigator.share){navigator.share({title:t('esp_share_title'),text:txt}).catch(function(){});}
...
function openEspEditor(mode){
  ...
  if(lbl)lbl.textContent=_espEdMode==='fiche'?t('esp_ed_fiche_title'):t('esp_ed_pub_title');
  var badge=g('espEdTypeBadge');
  if(badge){
    badge.textContent=_espEdMode==='fiche'?t('esp_badge_fiche'):t('esp_badge_pub');
  ...
  if(btn)btn.textContent=_espEdMode==='fiche'?t('esp_enregistrer'):t('esp_publier');
  var ed2=g('espAnnEditor');
  if(ed2)ed2.setAttribute('data-placeholder',_espEdMode==='fiche'?t('esp_fiche_content_ph'):t('esp_pub_placeholder'));
...
function openPubSettings(id,currentAcc){
  ...
  sheet.innerHTML='<div style="text-align:center;padding:12px 0 0"><div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;display:inline-block"></div></div>'
    +'<div style="font-size:13px;font-weight:700;color:var(--lite);text-align:center;padding:16px 24px 8px;text-transform:uppercase;letter-spacing:.06em">'+t('pub_settings_title')+'</div>'
    ...
    +'<div style="flex:1"><div style="font-size:15px;font-weight:700;color:var(--ink)">'+t('pub_visible_all')+'</div><div style="font-size:12px;color:var(--lite);margin-top:2px">'+t('pub_visible_all_sub')+'</div></div>'
    ...
    +'<div style="flex:1"><div style="font-size:15px;font-weight:700;color:var(--ink)">'+t('pub_private_label')+'</div><div style="font-size:12px;color:var(--lite);margin-top:2px">'+t('pub_private_sub')+'</div></div>'
    +(!isPublic?'<svg viewBox="0 0 24 24" fill="none" stroke="var(--or)" stroke-width="2.5" stroke-linecap="round" width="18" height="18"><polyline points="20 6 9 17 4 12"/></svg>':'')
    +'</button>'
    ...
    +'<div style="font-size:15px;font-weight:700;color:#EF4444">'+t('pub_delete')+'</div>'
    +'</button>'
    +'</div>'
    +'<div style="padding:0 12px"><button onclick="this.closest(\'[style*=fixed]\').remove();" style="width:100%;padding:15px;background:var(--bg);border:none;border-radius:16px;font-family:inherit;font-size:15px;font-weight:600;color:var(--mid);cursor:pointer">'+t('btn_annuler')+'</button></div>';
  bd.appendChild(sheet);document.body.appendChild(bd);
}
...
function openSondageSheet(){
  ...
  sheet.innerHTML='<div style="text-align:center;margin-bottom:16px"><div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;display:inline-block"></div></div>'
    ...
    +'<div><div style="font-size:18px;font-weight:800;color:var(--ink);letter-spacing:-.02em">'+t('sondage_title')+'</div>'
    +'<div style="font-size:13px;color:var(--lite);margin-top:2px">'+t('sondage_question_desc')+'</div></div>'
    +'</div>'
    +'<input id="_sdgQ" type="text" placeholder="'+escH(t('sondage_question_ph'))+'" class="esp-input" style="margin-bottom:14px">'
    ...
    +'<div id="_sdgOpts">'
    +'<input class="esp-input _sdgOpt" placeholder="'+escH(t('sondage_opt1_ph'))+'" style="margin-bottom:8px">'
    +'<input class="esp-input _sdgOpt" placeholder="'+escH(t('sondage_opt2_ph'))+'" style="margin-bottom:8px">'
    +'</div>'
    +'<button onclick="addSondageOption()" style="background:none;border:1.5px dashed var(--bdr);border-radius:12px;width:100%;padding:10px;font-family:inherit;font-size:13px;font-weight:600;color:var(--mid);cursor:pointer;margin-bottom:16px;-webkit-tap-highlight-color:transparent">'+t('sondage_add_option')+'</button>'
    +'<button onclick="submitSondage(this)" class="esp2-btn-submit" style="width:100%">'+t('sondage_publish_btn')+'</button>'
  ...
  Promise.all([
    ...
    if(!fiches.length&&!resources.length&&!content.length){
      grid.innerHTML='<div style="text-align:center;padding:60px 24px">'
        ...
        +'</div>'
        +'<div style="font-size:20px;font-weight:800;color:var(--ink);margin-bottom:10px;letter-spacing:-.03em">'+t('biblio_empty_title')+'</div>'
        +'<div style="font-size:14px;color:var(--lite);line-height:1.7">'+t('biblio_empty_sub_eleve')+'</div></div>';
      return;
    }
    var html='';
    function _elveLockedCard(title,icoHtml,onClickAttr){
      return'<div onclick="'+onClickAttr+'" style="position:relative;border-radius:16px;overflow:hidden;margin-bottom:8px;cursor:pointer;-webkit-tap-highlight-color:transparent">'
        ...
        +'<div style="font-size:12px;font-weight:700;color:var(--ink)">'+t('biblio_pw_required')+'</div>'
        +'</div></div>';
    }
    if(fiches.length){
      html+='<div class="mes-section-title" style="padding:4px 4px 8px">'+t('biblio_section_fiches')+'</div>';
      html+=fiches.map(function(f){
        ...
        return'<div onclick="espOpenFicheEleve(\''+pid+'\',\''+escH(f.id)+'\')" style="display:flex;align-items:center;gap:12px;background:var(--wh);border:none;box-shadow:0 3px 12px rgba(0,0,0,.1),0 0 0 0.5px rgba(0,0,0,.06);border-radius:16px;padding:14px;margin-bottom:8px;cursor:pointer;-webkit-tap-highlight-color:transparent">'
          ...
          +'<div style="font-size:12px;color:var(--lite);margin-top:2px">'+t('biblio_fiche_subtitle')+'</div></div>'
    ...
    if(resources.length||content.length){
      html+='<div class="mes-section-title" style="padding:'+(fiches.length?'16px':'4px')+' 4px 8px">'+t('biblio_section_ressources')+'</div>';
      resources.forEach(function(r){
        ...
        if(r.access_type==='password'){
          html+=_elveLockedCard(esc(r.title||t('biblio_doc_default')),icoHtml,'elveUnlockSheet(\''+pid+'\',\'resource\',\''+escH(r.id)+'\',\''+escH(r.url||'')+'\')');
        }else{
          html+='<a href="'+esc(r.url)+'" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:12px;background:var(--wh);border:none;box-shadow:0 3px 12px rgba(0,0,0,.1),0 0 0 0.5px rgba(0,0,0,.06);border-radius:16px;padding:14px;margin-bottom:8px;text-decoration:none;-webkit-tap-highlight-color:transparent">'
            +icoHtml
            +'<div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:700;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(r.title||t('biblio_doc_default'))+'</div>'
      ...
      content.forEach(function(c){
        ...
        if(c.access_type==='password'&&!isUnlocked){
          html+=_elveLockedCard(esc(c.title||t('biblio_content_default')),icoHtml,'elveUnlockSheet(\''+pid+'\',\'content\',\''+escH(c.id)+'\')');
        }else{
          html+='<div style="background:var(--wh);border:none;box-shadow:0 3px 12px rgba(0,0,0,.1),0 0 0 0.5px rgba(0,0,0,.06);border-radius:16px;padding:14px;margin-bottom:8px">'
            ...
            +'<div style="flex:1;min-width:0"><div style="font-size:14px;font-weight:700;color:var(--ink);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(c.title||t('biblio_content_default'))+'</div>'
            +'<div style="font-size:11px;font-weight:600;color:#10B981;margin-top:2px">'+t('biblio_access_free')+'</div></div></div>';
          if(isUnlocked&&c.content_url){
            html+='<a href="'+esc(c.content_url)+'" target="_blank" rel="noopener" style="display:block;margin-top:10px;padding:9px;background:rgba(99,102,241,.08);border-radius:10px;text-align:center;font-size:13px;font-weight:700;color:#6366F1;text-decoration:none">'+t('biblio_see_content')+'</a>';
          }
          html+='</div>';
        }
      });
    ...
    grid.innerHTML=html;
  }).catch(function(){grid.innerHTML='<div style="text-align:center;padding:40px;color:var(--lite)">'+t('biblio_err_load')+'</div>';});
}
...
function elveUnlockSheet(pid,kind,id,extra){
  ...
  sheet.innerHTML='<div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;margin:0 auto 20px"></div>'
    ...
    +'<div><div style="font-size:18px;font-weight:800;color:var(--ink);letter-spacing:-.02em">'+t('biblio_content_protected')+'</div>'
    +'<div style="font-size:13px;color:var(--lite);margin-top:3px">'+t('biblio_pw_enter')+'</div></div></div>'
    ...
    +'<button id="_elveUnlockBtn" style="width:100%;background:var(--or);color:#fff;border:none;border-radius:14px;padding:15px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer;box-shadow:0 4px 14px rgba(255,107,43,.28);margin-bottom:10px">'+t('btn_deverrouiller')+'</button>'
    +'<button onclick="this.closest(\'[style*=fixed]\').remove()" style="width:100%;padding:12px;background:none;border:none;font-family:inherit;font-size:14px;font-weight:600;color:var(--lite);cursor:pointer">'+t('btn_annuler')+'</button>';
  ...
  btn.onclick=function(){
    ...
    if(kind==='content'){
      fetch(API+'/teacher/'+pid+'/content/'+id+'/unlock',{method:'POST',headers:apiH(),body:JSON.stringify({password:pw})})
        .then(function(r){return r.json();}).then(function(d){
          if(d.error){btn.disabled=false;btn.textContent=t('btn_deverrouiller');document.getElementById('_elveUnlockInp').style.boxShadow='0 0 0 2px #EF4444';toast(t('t_wrong_password'),'');haptic(2);return;}
          bd.remove();haptic(8);toast(t('t_content_unlocked'),'');loadElveBibliotheque(pid);
        }).catch(function(){btn.disabled=false;btn.textContent=t('btn_deverrouiller');});
...
function loadBibliotheque(){
  ...
  var ACCESS_LABEL={enrolled:t('biblio_access_enrolled'),followers:t('biblio_access_enrolled'),public:t('biblio_access_free'),private:t('biblio_access_private'),password:t('biblio_access_password'),share:t('biblio_access_share')};
  ...
  Promise.all([
    ...
    var resources=results[1]||[];
    var items=[];
    fiches.forEach(function(f){items.push({id:f.id,kind:'fiche',title:f.title||t('biblio_fiche_default'),type:'fiche',access:f.access_type||'enrolled',created_at:f.created_at});});
    resources.forEach(function(r){items.push({id:r.id,kind:'resource',title:r.title||t('biblio_doc_default'),type:r.type||'',access:r.access_level||'followers',created_at:r.created_at});});
    ...
    if(!items.length){
      grid.innerHTML='<div style="grid-column:1/-1;text-align:center;padding:60px 24px">'
        ...
        +'</div>'
        +'<div style="font-size:20px;font-weight:800;color:var(--ink);margin-bottom:10px;letter-spacing:-.03em">'+t('biblio_empty_title')+'</div>'
        +'<div style="font-size:14px;color:var(--lite);line-height:1.7">'+t('biblio_empty_sub_prof')+'</div>'
        +'</div>';
      return;
    }
...
function biblioOpenAccessSheet(kind,id,curAcc){
  ...
  var opts=kind==='fiche'
    ?[
      {val:'enrolled',label:t('biblio_access_enrolled'),sub:t('biblio_access_enrolled_sub'),svgPath:'<path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>',stroke:'#16A34A',bg:'rgba(34,197,94,.08)'},
      {val:'public',label:t('biblio_access_free'),sub:t('biblio_access_public_sub'),svgPath:'<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 010 20M12 2a15.3 15.3 0 000 20"/>',stroke:'#6366F1',bg:'rgba(99,102,241,.08)'},
      {val:'private',label:t('biblio_access_private'),sub:t('biblio_access_private_sub'),svgPath:'<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/>',stroke:'var(--mid)',bg:'var(--bg)'}
    ]
    :[
      {val:'followers',label:t('biblio_access_enrolled'),sub:t('biblio_access_enrolled_sub_res'),svgPath:'<path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>',stroke:'#16A34A',bg:'rgba(34,197,94,.08)'},
      {val:'public',label:t('biblio_access_free'),sub:t('biblio_access_public_sub'),svgPath:'<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 010 20M12 2a15.3 15.3 0 000 20"/>',stroke:'#6366F1',bg:'rgba(99,102,241,.08)'},
      {val:'private',label:t('biblio_access_private'),sub:t('biblio_access_private_sub'),svgPath:'<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/>',stroke:'var(--mid)',bg:'var(--bg)'}
    ];
  ...
  html+='<button onclick="this.closest(\'[style*=fixed]\').remove()" style="width:100%;margin-top:4px;padding:14px;background:none;border:none;font-family:inherit;font-size:15px;font-weight:600;color:var(--lite);cursor:pointer;-webkit-tap-highlight-color:transparent">'+t('btn_annuler')+'</button>';
...
function _biblioAskPassword(onDone){
  ...
  sheet.style.cssText='background:var(--wh);border-radius:24px 24px 0 0;width:100%;max-width:480px;padding:24px;padding-bottom:max(32px,env(safe-area-inset-bottom,32px))';
  sheet.innerHTML='<div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;margin:0 auto 20px"></div>'
    +'<div style="font-size:17px;font-weight:800;color:var(--ink);margin-bottom:4px;letter-spacing:-.02em">'+t('biblio_set_pw_title')+'</div>'
    +'<div style="font-size:13px;color:var(--lite);margin-bottom:18px">'+t('biblio_set_pw_sub')+'</div>'
    ...
    +'<button onclick="_biblioPwConfirm()" style="width:100%;background:var(--or);color:#fff;border:none;border-radius:14px;padding:15px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer;box-shadow:0 4px 14px rgba(255,107,43,.28)">'+t('biblio_confirm')+'</button>'
    +'<button onclick="this.closest(\'[style*=fixed]\').remove()" style="width:100%;margin-top:10px;padding:12px;background:none;border:none;font-family:inherit;font-size:14px;font-weight:600;color:var(--lite);cursor:pointer">'+t('btn_annuler')+'</button>';
...
function espCaMsgSubmit(){
  ...
  fetch(API+'/messages/groupe',{method:'POST',headers:apiH(),
    ...
    .then(function(r){return r.json();}).then(function(d){
      if(btn){btn.disabled=false;btn.textContent=t('msg_send_all');}
      if(d.error){toast(t('t_error'),d.error);return;}
      haptic(4);toast(t('t_message_sent_all'),'');
      g('espCaMsgText').value='';
      g('espCaMsgForm').style.display='none';
    }).catch(function(){if(btn){btn.disabled=false;btn.textContent=t('msg_send_all');}toast(t('t_net_error'),'');});
}
...
function toggleEsp2Card(id,section){
  ...
  if(opening){
    // Côté élève
    var pid=_curPrFull;
    if(section==='cours'){_loadMpfRessourcesEsp(pid);}
    // Côté prof
...
function togFP(){
  ...
  if(fol.has(id)){
    ...
    toast(t('t_desabonne'),t('t_desabonne_pre')+p.nm);
...
var CR_MAT_ICONS={
  ...
  ia:'<rect x="7" y="7" width="10" height="10" rx="1"/><line x1="7" y1="9" x2="5" y2="9"/><line x1="7" y1="12" x2="5" y2="12"/><line x1="7" y1="15" x2="5" y2="15"/><line x1="17" y1="9" x2="19" y2="9"/><line x1="17" y1="12" x2="19" y2="12"/><line x1="17" y1="15" x2="19" y2="15"/><line x1="9" y1="7" x2="9" y2="5"/><line x1="12" y1="7" x2="12" y2="5"/><line x1="15" y1="7" x2="15" y2="5"/><line x1="9" y1="17" x2="9" y2="19"/><line x1="12" y1="17" x2="12" y2="19"/><line x1="15" y1="17" x2="15" y2="19"/>',
  ...
  francais:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">FR</text>',
  anglais:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">EN</text>',
  espagnol:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">ES</text>',
  allemand:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">DE</text>',
  italien:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">IT</text>',
  portugais:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">PT</text>',
  arabe:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">AR</text>',
  chinois:'<text x="12" y="17" text-anchor="middle" font-size="15" font-weight="700" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">中</text>',
  japonais:'<text x="12" y="17" text-anchor="middle" font-size="15" font-weight="700" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">日</text>',
  russe:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">RU</text>',
  coreen:'<text x="12" y="17" text-anchor="middle" font-size="15" font-weight="700" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">한</text>',
  hindi:'<text x="12" y="16" text-anchor="middle" font-size="10" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">HI</text>',
  latin:'<text x="12" y="16" text-anchor="middle" font-size="8.5" font-weight="800" fill="currentColor" stroke="none" font-family="system-ui,-apple-system,sans-serif">LAT</text>',
  ...
  ecriture:'<path d="M17 3a2.121 2.121 0 013 3L7 19l-4 1 1-4L17 3z"/><line x1="15" y1="5" x2="19" y2="9"/>',
  theatre:'<circle cx="7.5" cy="8.5" r="5.5"/><path d="M5 9.5c.5 1.5 1.5 2 2.5 2s2-.5 2.5-2"/><circle cx="16.5" cy="13.5" r="5.5"/><path d="M14 13c.5-1.5 1.5-2 2.5-2s2 .5 2.5 2"/>',
  ...
  fitness:'<rect x="2" y="10" width="3" height="4" rx="1"/><rect x="19" y="10" width="3" height="4" rx="1"/><rect x="7" y="9" width="2" height="6" rx="1"/><rect x="15" y="9" width="2" height="6" rx="1"/><line x1="9" y1="12" x2="15" y2="12" stroke-width="3"/>',
  yoga:'<circle cx="12" cy="4" r="2"/><path d="M6.5 18h11M12 14l-5 4M12 14l5 4M12 7v7M7 10l5 4M17 10l-5 4"/>',
  martial:'<circle cx="12" cy="3" r="2"/><path d="M12 5v6M8 7l-3 5M16 7l3 5M9 11l-3 9M15 11l3 9"/>',
  ...
  echecs:'<path d="M12 2v4M10 4h4"/><path d="M9 8h6l1 6H8z"/><rect x="6" y="14" width="12" height="4" rx="1"/><rect x="4" y="18" width="16" height="4" rx="1"/>',
...
function _openDupSheet(c){
  ...
  var rows=[
    {k:t('dup_field_mat'),    v:_sd.matiere?subjL(_sd.matiere):'—'},
    {k:t('dup_field_niveau'), v:_sd.niveau||'—'},
    {k:t('dup_field_mode'),   v:_sd.mode==='visio'?t('mode_visio'):t('mode_pres')},
    {k:t('dup_field_prix'),   v:_sd.prix?_sd.prix+'€':'—'},
    {k:t('dup_field_places'), v:_sd.places||'—'},
    {k:t('dup_field_lieu'),   v:_sd.lieu||(_sd.mode==='visio'?t('dup_visio_auto'):'—')},
  ].map(function(r){
  ...
  var html='<div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;margin:14px auto 0"></div>'
    ...
    +'</div>'
    +'<div>'
    +'<div style="font-size:17px;font-weight:800;color:var(--ink);letter-spacing:-.03em">'+t('dup_title')+'</div>'
    ...
    +'<div style="text-align:left"><div style="font-size:15px;font-weight:700;color:#fff">'+t('dup_date_only')+'</div><div style="font-size:12px;color:rgba(255,255,255,.75);margin-top:2px">'+t('dup_date_sub')+'</div></div>'
    +'</button>'
    ...
    +'<div style="text-align:left"><div style="font-size:15px;font-weight:700;color:var(--ink)">'+t('dup_full')+'</div><div style="font-size:12px;color:var(--lite);margin-top:2px">'+t('dup_full_sub')+'</div></div>'
    +'</button>'
    +'<button onclick="closeQuickSheet()" style="width:100%;padding:13px;background:none;border:none;font-family:inherit;font-size:14px;font-weight:600;color:var(--lite);cursor:pointer">'+t('btn_annuler')+'</button>'
    +'</div>'
...
async function loadConversations(){
  ...
  try{
    ...
    var html=Object.keys(convs).map(function(otherId,_idx){
      ...
      else if(_pc.startsWith('%%ESP%%')){try{var _ep=JSON.parse(_pc.slice(7));preview=(_ep.t==='fiche'?t('msg_prv_fiche'):_ep.t==='pub'?t('msg_prv_pub'):_ep.t==='sondage'?t('msg_prv_sondage'):t('msg_prv_contenu'))+(_ep.title?' · '+esc(_ep.title.slice(0,22)):'');}catch(e){preview=t('msg_prv_contenu_p');}}
...
function _espMsgSendCard(type,id,title){
  ...
  haptic(6);toast(t('sondage_share'),'');
}
...
function openSondageCreator(){
  ...
  var html='<div style="width:36px;height:4px;background:var(--bdr);border-radius:4px;margin:14px auto 0"></div>'
    +'<div style="padding:14px 20px 8px"><div style="font-size:17px;font-weight:800;color:var(--ink)">'+t('sondage_title')+'</div></div>'
    +'<div style="padding:0 20px;display:flex;flex-direction:column;gap:10px">'
    +'<input id="sondageQ" placeholder="'+escH(t('sondage_question_ph'))+'" style="'+inpStyle+';font-size:15px" onfocus="this.style.borderColor=\'var(--or)\'" onblur="this.style.borderColor=\'var(--bdr)\'">'
    +'<input id="sondageO1" placeholder="'+escH(t('sondage_opt1_ph'))+'" style="'+inpStyle+'" onfocus="this.style.borderColor=\'var(--or)\'" onblur="this.style.borderColor=\'var(--bdr)\'">'
    +'<input id="sondageO2" placeholder="'+escH(t('sondage_opt2_ph'))+'" style="'+inpStyle+'" onfocus="this.style.borderColor=\'var(--or)\'" onblur="this.style.borderColor=\'var(--bdr)\'">'
    +'<input id="sondageO3" placeholder="'+escH(t('sondage_opt3_ph'))+'" style="'+inpStyle+'" onfocus="this.style.borderColor=\'var(--or)\'" onblur="this.style.borderColor=\'var(--bdr)\'">'
    +'</div>'
    +'<div style="padding:14px 20px max(20px,calc(env(safe-area-inset-bottom,0px)+16px));display:flex;flex-direction:column;gap:8px">'
    +'<button onclick="_sendSondageMsg()" style="width:100%;background:var(--or);color:#fff;border:none;border-radius:14px;padding:15px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer;box-shadow:0 4px 14px rgba(255,107,43,.3)">'+t('sondage_send')+'</button>'
    +'<button onclick="closeQuickSheet()" style="width:100%;background:var(--bg);color:var(--mid);border:none;border-radius:14px;padding:14px;font-family:inherit;font-weight:600;font-size:15px;cursor:pointer">'+t('btn_annuler')+'</button>'
    +'</div>';
  showQuickSheet(html);
}
...
function _sendSondageMsg(){
  ...
  if(!q||!o1||!o2){toast(t('sondage_err'),'');return;}
...
function showBadgeInfo(type){
  ...
  var info={
    identite:{
      ...
      name:t('badge_identite_name'),
      badge:t('badge_identite_rarity'),
      desc:t('badge_identite_desc'),
      how:t('badge_identite_how')
    },
    diplome:{
      ...
      name:t('badge_diplome_name'),
      badge:t('badge_diplome_rarity'),
      desc:t('badge_diplome_desc'),
      how:t('badge_diplome_how')
    },
    confiance:{
      ...
      name:t('badge_confiance_name'),
      badge:t('badge_confiance_rarity'),
      desc:t('badge_confiance_desc'),
      how:t('badge_confiance_how')
    }
  };
  ...
  content.innerHTML=
    ...
    +'<div style="font-size:10px;font-weight:800;letter-spacing:.14em;color:rgba(255,255,255,.7);text-transform:uppercase;margin-bottom:16px">✦ '+t('badge_certified_label')+' ✦</div>'
    ...
    +'<div style="font-size:11px;font-weight:800;color:var(--lite);text-transform:uppercase;letter-spacing:.09em;margin-bottom:8px">'+t('badge_what_label')+'</div>'
    ...
    +'<div style="font-size:11px;font-weight:800;color:var(--lite);text-transform:uppercase;letter-spacing:.09em;margin-bottom:8px">'+t('badge_how_label')+'</div>'
    +'<div style="font-size:13.5px;color:var(--mid);line-height:1.65">'+d.how+'</div>'
    +'</div>':'')
    +'<button onclick="closeBadgeInfo()" style="width:100%;background:var(--bg);color:var(--ink);border:none;border-radius:14px;padding:14px;font-family:inherit;font-weight:700;font-size:15px;cursor:pointer;margin-top:10px">'+t('btn_fermer')+'</button>';
  bd.style.display='flex';
  document.body.style.overflow='hidden';
}
  ...
  try {
    ...
    el.innerHTML = data.map(function(p) {
      ...
      return '<div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-bottom:1px solid var(--bdr)">'
        + '<div style="width:42px;height:42px;border-radius:12px;background:linear-gradient(135deg,rgba(34,192,105,.15),rgba(34,192,105,.05));display:flex;align-items:center;justify-content:center;flex-shrink:0">'
        ...
        + '</div>'
        + '</div>'
        + '<div style="font-size:16px;font-weight:800;color:#22C069">'+p.amount.toFixed(2)+'€</div>'
        + '</div>';
    }).join('');
  } catch(e) {
...
async function submitContact(){
  ...
  try{
    ...
    if(r.ok){
      ...
      toast(t('t_msg_envoye'),t('t_msg_envoye_sub'));
    } else {
...
function buildStepDOM(){
  ...
  div.innerHTML=
    ...
    +'</div>'
    +'<div style="padding:0 20px 14px;flex-shrink:0"><div style="height:6px;background:var(--bg);border-radius:50px;box-shadow:inset 0 1px 3px rgba(0,0,0,.12);overflow:hidden"><div id="stepFill" style="height:100%;background:linear-gradient(90deg,#FF9D66,#FF4500);border-radius:50px;box-shadow:0 2px 6px rgba(255,69,0,.45);transition:width .4s cubic-bezier(.22,.61,.36,1);width:0%"></div></div></div>'
  ...
  if(step.id==='mode'){
    ...
    if(_sd.prive){
      html+='<div style="margin-top:16px;width:100%;background:var(--orp);border-radius:14px;padding:14px 16px">'
        +'<div style="font-size:11px;font-weight:700;color:var(--lite);text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px">'+t('cr_step_code_acces')+'</div>'
    ...
    html+='<div class="search-bar-premium" style="width:100%;padding:14px 18px"><input class="search-input-premium" id="stepTitre" style="font-size:18px;font-weight:600;color:var(--ink)" type="text" placeholder="'+escH(t('cr_step_titre_ph'))+'" value="'+escH(_sd.titre)+'"></div>';
    ...
    html+='<div class="search-bar-premium" style="width:100%;margin-bottom:16px">'
      ...
      +'<input class="search-input-premium" id="stepMatSearch" type="search" placeholder="'+escH(t('cr_step_mat_ph'))+'" autocomplete="off">'
      +'</div>';
    ...
    html+='<div style="width:100%;display:flex;flex-direction:column;gap:16px">'
      ...
      +'<div style="flex:1;min-width:140px"><label style="'+lbl+'">'+t('cr_step_date_lbl')+'</label><div class="search-bar-premium" style="padding:12px 14px"><input class="search-input-premium" id="stepDate" style="font-size:15px;font-weight:500;color:var(--ink)" type="date" min="'+today+'" value="'+escH(_sd.date)+'"></div></div>'
      +'<div style="flex:1;min-width:140px"><label style="'+lbl+'">'+t('cr_step_heure_lbl')+'</label><div class="search-bar-premium" style="padding:12px 14px"><input class="search-input-premium" id="stepHeure" style="font-size:15px;font-weight:500;color:var(--ink)" type="time" value="'+escH(_sd.heure)+'"></div></div>'
      +'</div>'
      +'<div><label style="'+lbl+'">'+t('cr_step_duree_lbl')+'</label><div class="search-bar-premium" style="padding:12px 14px"><input class="search-input-premium" id="stepDuree" style="font-size:15px;font-weight:500;color:var(--ink);text-align:center" type="number" value="'+_sd.duree+'" min="30"></div></div>'
      +'</div>';
    ...
    if(_sd.mode==='visio'){
      ...
      var _lieuCfg={
        domicile:{
          label2:t('cr_step_dom_lbl'),
          ph2:t('cr_step_dom_ph'),
          note2:t('cr_step_dom_note')
        },
        etablissement:{
          label2:t('cr_step_etab_lbl'),
          ph2:t('cr_step_etab_ph'),
          note2:t('cr_step_etab_note')
        },
        autre:{
          label2:t('cr_step_dom_lbl'),
          ph2:t('cr_step_autre_ph'),
          note2:t('cr_step_autre_note')
        }
      };
      ...
      html+='<div style="width:100%;display:flex;flex-direction:column;gap:10px">'
        ...
        +(_cfg?'<div id="stepLieuInputWrap" style="display:flex;flex-direction:column;gap:12px">'
          +'<div>'
            +'<div style="font-size:11px;font-weight:700;color:var(--lite);letter-spacing:.06em;text-transform:uppercase;margin-bottom:8px">'+t('cr_step_ville_lbl')+'</div>'
            +'<div class="search-bar-premium" style="padding:12px 14px"><input class="search-input-premium" id="stepLieu" style="font-size:15px;font-weight:500;color:var(--ink)" type="text" placeholder="'+escH(t('cr_step_ville_ph'))+'" value="'+escH(_sd.lieu||'')+'" enterkeyhint="next"></div>'
            ...
            +'<div style="margin-top:7px;font-size:11.5px;color:var(--lite);line-height:1.45">'+t('cr_step_ville_note')+'</div>'
          +'</div>'
          +'<div>'
    ...
    html+='<div style="width:100%;display:flex;flex-direction:column;gap:28px">'
      ...
      // Places
      +'<div style="text-align:center">'
      +'<div style="'+_stpLbl+'">'+t('cr_step_places_lbl')+'</div>'
      +'<div style="display:flex;align-items:center;justify-content:center;gap:20px">'
      ...
      +'</div>'
      // Prix par élève
      +'<div id="stepPrixCalc" style="background:var(--orp);border-radius:16px;padding:16px;text-align:center;border:1px solid rgba(255,107,43,.12);'+(_sd.prix>0?'':'display:none;')+'"><div style="font-size:12px;color:var(--mid);margin-bottom:4px">'+t('cr_step_ppeleve_lbl')+'</div><div id="stepPrixCalcVal" style="font-size:32px;font-weight:800;color:var(--or)">'+(_sd.prix>0?Math.ceil(_sd.prix/Math.max(1,_sd.places))+'\u20ac':'-')+'</div></div>'
      +'</div>';

  }else if(step.id==='desc'){
    html+='<div style="width:100%;display:flex;flex-direction:column;gap:8px">'
      +'<div class="search-bar-premium" style="padding:14px 16px;align-items:flex-start"><textarea class="search-input-premium" id="stepDesc" rows="6" placeholder="'+escH(t('cr_step_desc_ph'))+'" style="resize:none;font-size:15px;font-weight:400;min-height:160px;line-height:1.6;color:var(--ink)">'+escH(_sd.desc)+'</textarea></div>'
    ...
    var _recOpts=[
      {val:'once',     label:t('rec_once'),     sub:t('rec_once_sub')},
      {val:'weekly',   label:t('rec_weekly'),   sub:t('rec_weekly_sub')},
      {val:'biweekly', label:t('rec_biweekly'), sub:t('rec_biweekly_sub')},
      {val:'monthly',  label:t('rec_monthly'),  sub:t('rec_monthly_sub')},
    ];
    ...
    html+='<div id="recCountSec" style="margin-top:20px;text-align:center;'+(_sd.recurrence==='once'?'display:none;':'')+'width:100%">'
      +'<div style="font-size:11px;font-weight:700;color:var(--lite);letter-spacing:.08em;text-transform:uppercase;margin-bottom:12px">'+t('cr_step_seances_lbl')+'</div>'
      +'<div style="display:flex;align-items:center;justify-content:center;gap:20px">'
...
function _calDay(d){return d.toLocaleDateString(_dateLocale(),{weekday:'short'}).replace(/[.,]/g,'').replace(/^\w/,function(c){return c.toUpperCase();});}
function _calMonth(d){return d.toLocaleDateString(_dateLocale(),{month:'short'}).replace(/[.,]/g,'');}
...
function _calBuildHeader(myCours){
  ...
  var titleStr=_mesSeg==='past'
    ? t('cal_past_title')
    : _calDay(selD)+' '+selD.getDate()+' '+_calMonth(selD);
  ...
  if(_mesSeg!=='past'){
    ...
    for(var i=0;i<7;i++){
      ...
      chipsHtml+='<button class="'+cls+'" data-ymd="'+ymd+'" onclick="calSelectDay(\''+ymd+'\')">'
        +'<span class="cal-chip-lbl">'+_calDay(d)+'</span>'
...
function openShareCoursSheet(){
  ...
  if(!myC.length){toast(t('empty_resa_title'),t('t_aucun_cours_share_sub'));return;}
  var bd=document.createElement('div');
  bd.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);z-index:900;display:flex;align-items:flex-end;justify-content:center';
  bd.onclick=function(e){if(e.target===bd)bd.remove();};
  var sheet=document.createElement('div');
...
// ============================================================
var ACC_TITLES={};
function _initAccTitles(){ACC_TITLES={'R':t('acc_tab_cours'),'F':t('acc_tab_suivis'),'H':t('acc_tab_histo'),'P':t('acc_tab_profil'),'Rev':t('acc_tab_revenus'),'Rmb':t('acc_tab_remb'),'Avis':t('acc_tab_avis')};}
_initAccTitles();
...
async function exportMesDonnees(){
  ...
  var filename='courspool-mes-donnees-'+new Date().toISOString().slice(0,10)+'.json';
  var blob=new Blob([json],{type:'application/json'});
  // Fallback Web Share API (iOS Capacitor) si disponible
  if(navigator.canShare&&navigator.canShare({files:[new File([blob],filename,{type:'application/json'})]})){
    navigator.share({files:[new File([blob],filename,{type:'application/json'})],title:'Mes données CoursPool'}).catch(function(){});
  }else{
    var url=URL.createObjectURL(blob);
  ...
  haptic(6);
  toast(t('t_export_done'),t('t_preparing_sub'));
}
...
function _updateSearchPill(val,sub){
  ...
  if(val){
    ...
    if(subEl){subEl.textContent=sub||t('search_sub');subEl.style.display='block';}
    ...
    if(main){main.textContent=t('search_title');main.style.color='#222';}
...
function _vOpenDemo(){
  ...
  // Populate grid — uniquement l'utilisateur local
  ...
  _vApplyLayout();
  // Démarrer l'analyse audio (réutilise le stream s'il est encore actif)
  _ensureDemoAudio(_startDemoAudio);
  haptic(1);
}

function _ensureDemoAudio(callback){
  // Réutiliser le stream existant si toujours actif (évite de re-demander la permission)
  if(_demoAudioStream&&_demoAudioStream.active&&
     _demoAudioStream.getAudioTracks().some(function(t){return t.readyState==='live';})){
    callback(_demoAudioStream);return;
  }
  if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia)return;
  navigator.mediaDevices.getUserMedia({audio:true,video:false})
    .then(function(stream){callback(stream);})
...
function _vBuildDemoTile(f){
  ...
  micInd.innerHTML=_vMicSvgSm(f.local?_localMuted:false);
  if(f.local){
...
function _buildVisioHTML(){
  ...
  return ''
    ...
    +'</div>'
    // Central zone: video grid + people panel
...
function _vGetPipFeaturedSid(sids){
  if(!sids||!sids.length)return'local';
  // Active non-local speaker wins; if local is speaking → show a non-local participant
  if(_activeSpeakerSid&&_activeSpeakerSid!=='local'&&sids.indexOf(_activeSpeakerSid)!==-1)return _activeSpeakerSid;
  var nonLocal=sids.filter(function(s){return s!=='local';});
  if(nonLocal.length)return nonLocal[0];
  return'local';
}
function _vApplyLayout(){
  var grid=g('_vGrid');if(!grid)return;
  var tiles=Array.prototype.slice.call(grid.querySelectorAll('[id^="_vt-"]'));
  var n=tiles.length;
  var sids=tiles.map(function(t){return t.id.replace('_vt-','');});

  // ── PiP mode: board open → one speaker fills the bubble ──
  if(_boardActive){
    ...
    if(newFeat!==_vPipFeaturedSid){
      ...
      var newEl=g('_vt-'+newFeat);
      _vPipFeaturedSid=newFeat;
      if(oldEl&&newEl&&oldEl!==newEl){
        oldEl.style.transition='opacity .22s';
...
function _vToggleMic(){
  ...
  if(_isDemoMode){
    if(_localMuted){
      // Couper le micro : stopper l'analyse sans stopper le stream (évite re-prompt permission)
      if(_demoRAFId){cancelAnimationFrame(_demoRAFId);_demoRAFId=null;}
      _demoAnalyser=null;
      if(_demoAudioCtx){try{_demoAudioCtx.close();}catch(e){}_demoAudioCtx=null;}
      ...
      _ensureDemoAudio(function(stream){
        _startDemoAudio(stream);
        var ban=g('_vMutedBanner');if(ban)ban.style.display='none';
      });
    }
    haptic(1);return;
  }
...
function _buildBoardInner(){
  ...
  return ''
    ...
    +'<span style="font-size:11px;font-weight:600;color:#facc15;">Lecture seule</span>'
    +'</div>'
    // Panneau permissions prof (liste participants + boutons Autoriser)
    +(_isOwner?'<div id="_brdPermPanel" style="display:none;position:fixed;right:12px;top:max(calc(env(safe-area-inset-top,20px)+72px),92px);z-index:200;background:rgba(20,20,35,.92);backdrop-filter:blur(12px);border-radius:14px;padding:10px 12px;min-width:160px;box-shadow:0 4px 24px rgba(0,0,0,.4);"></div>':'')
...
function _boardRenderSubbar(){
  ...
  var h='';
  // Lecture seule : dimmer les outils de dessin
  var readOnly=(_brdRoomId&&!_brdCanEdit);
  if(readOnly)sub.style.opacity='0.45';else sub.style.opacity='';
  ...
  if(_brdTool==='shape'){
    ...
    shapes.forEach(function(sh){
      ...
      h+='<button onclick="_boardSetShape(\''+sh.t+'\')" style="background:'+(a?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">'
        +'<svg viewBox="0 0 24 24" fill="none" stroke="'+(a?'#FF6B2B':ic)+'" stroke-width="1.9" stroke-linecap="round" width="17" height="17">'+sh.svg+'</svg></button>';
    });
    h+=sep;
    // Fill toggle
    h+='<button onclick="_boardToggleShFill()" title="Remplissage" style="background:'+(_brdShFill?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">'
      +'<svg viewBox="0 0 24 24" fill="'+(_brdShFill?'rgba(255,255,255,.55)':'none')+'" stroke="'+(_brdShFill?'#FF6B2B':ic)+'" stroke-width="1.8" stroke-linecap="round" width="17" height="17"><rect x="3" y="3" width="18" height="18" rx="2"/></svg></button>';
    h+=sep;
    // Stroke sizes
    [{sz:2,h:2},{sz:5,h:5},{sz:11,h:9}].forEach(function(s){
      var a=_brdSz===s.sz;
      h+='<button onclick="_boardSetSize('+s.sz+')" style="background:'+(a?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:40px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">'
        +'<div style="width:28px;height:'+s.h+'px;background:'+(a?_brdColor:'rgba(255,255,255,.4)')+';border-radius:'+(s.h/2)+'px;"></div></button>';
    });
...
function _vOpenBoard(){
  ...
  _boardActive=true;
  var bdV=g('bdVisio');if(!bdV)return;
  var grid=g('_vGrid');if(!grid)return;
  if(!_brdPages.length)_brdPages.push(null);
  // ── Init room collaborative ──────────────────────────────────
  _brdRoomId=null;_brdCanEdit=false;_brdParticipants=[];
  if(!_isDemoMode&&typeof _socket!=='undefined'&&_socket&&_socket.connected&&_visioCurrentUrl){
    var _rId=_visioCurrentUrl.split('/').pop();
    if(_rId){
      _brdRoomId=_rId;
      var _uName=(user&&(user.prenom||user.email))||'?';
      if(_isOwner){
        _brdCanEdit=true;
        _socket.emit('board_init',{roomId:_brdRoomId,userName:_uName});
      }else{
        _socket.emit('board_join',{roomId:_brdRoomId,userName:_uName});
      }
    }
  }else if(_isDemoMode){
    _brdCanEdit=true; // démo = local, tout autorisé
  }
  // PiP setup
  var sw=_PW[_pipSzIdx],sh=_PH[_pipSzIdx];
  if(_pipX===null)_pipX=window.innerWidth-sw-20;
...
function _vCloseBoard(){
  ...
  _brdRoomId=null;_brdCanEdit=false;_brdParticipants=[];
  _brdBgType='grid';
  _brdCleanObjSel();_brdObjects=[];_brdObjPages=[];
  ...
  if(pip&&grid){
    ...
    if(main)main.insertBefore(grid,people||null);
    pip.remove();
  }
  if(grid){grid.style.cssText='flex:1;display:grid;gap:8px;overflow:hidden;background:#0d0d18;padding:8px;min-height:0;';_vApplyLayout();}
  var btn=g('_vBoardBtn');if(btn){btn.style.background='rgba(255,255,255,.12)';btn.style.boxShadow='';}
  haptic(1);
}
...
function _boardDeletePage(idx){
  ...
  _brdCleanObjSel();
  _brdPages.splice(idx,1);_brdPageNames.splice(idx,1);_brdPageHists.splice(idx,1);_brdObjPages.splice(idx,1);
  if(_brdPageIdx>=_brdPages.length)_brdPageIdx=_brdPages.length-1;
  _brdObjects=_brdObjPages[_brdPageIdx]?JSON.parse(JSON.stringify(_brdObjPages[_brdPageIdx])):[];
...
function _boardGoPage(idx){
  ...
  // Sauvegarder objets + hist de la page courante
  ...
  _brdPageIdx=idx;
  // Restaurer objets de la page destination
...
function _boardAddPage(){
  ...
  _brdObjPages[_brdPageIdx]=JSON.parse(JSON.stringify(_brdObjects));
  _brdPageHists[_brdPageIdx]={hist:_brdHist.slice(),idx:_brdHistIdx};
  _brdPages.push(null);_brdPageNames.push('');_brdObjPages.push([]);
  _brdPageIdx=_brdPages.length-1;_brdObjects=[];
...
function _boardToolTap(tool){
  ...
  if(_brdSel.active)_brdCommitSel();
  if(_brdObjSel.active&&tool!=='select')_brdCleanObjSel();
...
function _brdToggleBold(){_brdTextBold=!_brdTextBold;_boardRenderSubbar();_brdRenderActiveTxtBar();_brdUpdateActiveText();}
function _brdToggleItalic(){_brdTextItalic=!_brdTextItalic;_boardRenderSubbar();_brdRenderActiveTxtBar();_brdUpdateActiveText();}
...
function _brdBuildTxtBarHTML(){
  ...
  var sep='<div style="width:1px;height:20px;background:rgba(255,255,255,.15);margin:0 3px;flex-shrink:0;"></div>';
  var h='';
  [{sz:12,lbl:'S'},{sz:16,lbl:'M'},{sz:22,lbl:'L'},{sz:30,lbl:'XL'}].forEach(function(s){
    var a=_brdTextSize===s.sz;
    h+='<button onmousedown="event.preventDefault();_brdSetTextSz('+s.sz+')" ontouchstart="event.preventDefault();_brdSetTextSz('+s.sz+')" style="background:'+(a?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:30px;height:28px;border-radius:7px;font-family:inherit;font-size:11px;font-weight:600;color:'+(a?'#fff':'rgba(255,255,255,.5)')+';-webkit-tap-highlight-color:transparent;">'+s.lbl+'</button>';
  });
  h+=sep;
  h+='<button onmousedown="event.preventDefault();_brdToggleBold()" ontouchstart="event.preventDefault();_brdToggleBold()" style="background:'+(_brdTextBold?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:28px;height:28px;border-radius:7px;font-family:inherit;font-size:14px;font-weight:700;color:'+(_brdTextBold?'#fff':'rgba(255,255,255,.5)')+';-webkit-tap-highlight-color:transparent;">B</button>';
  h+='<button onmousedown="event.preventDefault();_brdToggleItalic()" ontouchstart="event.preventDefault();_brdToggleItalic()" style="background:'+(_brdTextItalic?'rgba(255,255,255,.18)':'transparent')+';border:none;cursor:pointer;width:28px;height:28px;border-radius:7px;font-family:inherit;font-size:14px;font-style:italic;font-weight:600;color:'+(_brdTextItalic?'#fff':'rgba(255,255,255,.5)')+';-webkit-tap-highlight-color:transparent;"><i>I</i></button>';
  h+=sep;
  var alignSvgs={left:'<line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="15" y2="12"/><line x1="3" y1="18" x2="18" y2="18"/>',center:'<line x1="3" y1="6" x2="21" y2="6"/><line x1="6" y1="12" x2="18" y2="12"/><line x1="4" y1="18" x2="20" y2="18"/>',right:'<line x1="3" y1="6" x2="21" y2="6"/><line x1="9" y1="12" x2="21" y2="12"/><line x1="6" y1="18" x2="21" y2="18"/>'};
  ['left','center','right'].forEach(function(al){
    var a=_brdTextAlign===al;
...
function _boardDown(e){
  ...
  if(_brdTool==='pen'){
    ...
    _brdSnap=null;_brdSS=null; // reset rectangle select
    var _hitId=_brdHitTest(p.x,p.y);
    // Double-tap sur un texte → édition
    var _now2=Date.now();
    if(_hitId&&_now2-_brdDblTapT<350&&_brdDblTapId===_hitId){
      _brdDblTapT=0;_brdDblTapId=null;
      var _tObj=_brdObjects.find(function(o){return o.id===_hitId;});
      if(_tObj&&_tObj.type==='text'){_brdEditTextObj(_tObj);_brdDraw=false;return;}
    }
...
function _boardMove(e){
  ...
  if(_brdTool==='pen'||_brdTool==='marker'){
    ...
    // Pression stylet : moduler l'épaisseur du trait en temps réel
    if(e.pointerType==='pen'&&e.pressure>0){
      var pr=Math.max(0.3,e.pressure);
      _brdX.lineWidth=(_brdTool==='marker'?_brdSz*2.5:_brdSz)*pr;
    }
    _brdPts.push(p);var n=_brdPts.length;
    if(n>=3){
      var p1=_brdPts[n-2],p2=_brdPts[n-1];
      var mx=(p1.x+p2.x)/2,my=(p1.y+p2.y)/2;
      _brdX.quadraticCurveTo(p1.x,p1.y,mx,my);_brdX.stroke();
    ...
    }else if(_brdSS&&_brdSnap){
      // Rectangle de sélection
      _brdRestoreSnap(_brdSnap);
      _brdX.save();_brdX.strokeStyle='#FF6B2B';_brdX.lineWidth=2/_brdZoom;
...
function _boardUp(e){
  ...
  if(_brdSnapTimer){clearTimeout(_brdSnapTimer);_brdSnapTimer=null;}
  // Masquer l'indicateur snap si le doigt se lève avant le déclenchement
  var _sr=document.getElementById('_brdSnapRing');
  if(_sr){_sr.style.opacity='0';_sr.style.transform='translate(-50%,-50%) scale(0)';}
  ...
  if(_brdTool==='pen'||_brdTool==='marker'||_brdTool==='eraser'){
    // Enregistrer l'objet dans le modèle
    if(_brdPts.length>=2){
      var _oid=_brdObjId();
      var _dPtsRaw=_brdDecimate(_brdPts,3);
      if(_brdTool==='eraser'){
        _brdObjects.push({id:_oid,type:'erase',pts:_dPtsRaw,size:_brdEr});
      }else{
        _brdObjects.push({id:_oid,type:'stroke',tool:_brdTool,pts:_dPtsRaw,color:_brdColor,size:_brdSz});
      }
    }
    _boardSaveHist();
    ...
    if(_brdRoomId&&_brdCanEdit&&_brdPts.length>=2){
      ...
      var _emitId=_brdObjects.length?_brdObjects[_brdObjects.length-1].id:_brdObjId();
      _brdEmitOp(_brdTool==='eraser'
        ?{type:'erase',id:_emitId,pts:_dPts,size:_brdEr}
        :{type:'stroke',id:_emitId,tool:_brdTool,pts:_dPts,color:_brdColor,size:_brdSz});
    }
  }else if(_brdTool==='shape'&&_brdSnap&&_brdSS){
    var p=_boardGetPos(e);
    var _ssx=_brdSS.x,_ssy=_brdSS.y;
    _brdRestoreSnap(_brdSnap);
    _boardDrawShape(_ssx,_ssy,p.x,p.y);
    var _shId=_brdObjId();
    _brdObjects.push({id:_shId,type:'shape',shType:_brdShType,x1:_ssx,y1:_ssy,x2:p.x,y2:p.y,color:_brdColor,size:_brdSz,fill:_brdShFill});
    _brdSnap=null;_brdSS=null;_boardSaveHist();
    if(_brdRoomId&&_brdCanEdit){
      _brdEmitOp({type:'shape',id:_shId,shType:_brdShType,x1:_ssx,y1:_ssy,x2:p.x,y2:p.y,color:_brdColor,size:_brdSz,fill:_brdShFill});
    }
  }else if(_brdTool==='select'){
    if(_brdObjSel.dragging){
      var dx=_brdObjSel.dragDx,dy=_brdObjSel.dragDy;
      _brdObjSel.dragging=false;_brdObjSel.bgSnap=null;
      if(Math.hypot(dx,dy)>3){
        _boardSaveHist();
        _brdObjSel.ids.forEach(function(id){
          var idx=_brdObjects.findIndex(function(o){return o.id===id;});
          if(idx!==-1)_brdObjects[idx]=_brdObjTranslated(_brdObjects[idx],dx,dy);
        });
        _brdRenderAll();_boardSavePage();_boardSaveHist();
        if(_brdRoomId&&_brdCanEdit)_brdEmitOp({type:'objmove',ids:_brdObjSel.ids.slice(),dx:dx,dy:dy});
      }else{
        _brdRenderAll(); // tiny move = tap, pas de déplacement
      }
    ...
    if(_brdSS&&_brdSnap){
      ...
      if(sw>15&&sh>15){
        var _rIds=_brdRectHitObjs(sx,sy,sw,sh);
        if(_rIds.length)_brdActivateObjSel(_rIds);
      }
    }
  }
  _brdPts=[];
}
function _boardDrawShape(x1,y1,x2,y2){
  _brdX.beginPath();
  _brdX.strokeStyle=_brdColor;_brdX.lineWidth=_brdSz;
  _brdX.lineCap='round';_brdX.lineJoin='round';
  var mx=(x1+x2)/2,my=(y1+y2)/2,rx=Math.abs(x2-x1)/2,ry=Math.abs(y2-y1)/2;
  if(_brdShType==='rect')       {_brdX.rect(Math.min(x1,x2),Math.min(y1,y2),Math.abs(x2-x1),Math.abs(y2-y1));}
  else if(_brdShType==='circle'){_brdX.ellipse(mx,my,rx,ry,0,0,Math.PI*2);}
  else if(_brdShType==='tri')   {_brdX.moveTo(mx,y1);_brdX.lineTo(x2,y2);_brdX.lineTo(x1,y2);_brdX.closePath();}
  else if(_brdShType==='diamond'){_brdX.moveTo(mx,y1);_brdX.lineTo(x2,my);_brdX.lineTo(mx,y2);_brdX.lineTo(x1,my);_brdX.closePath();}
  else if(_brdShType==='line')  {_brdX.moveTo(x1,y1);_brdX.lineTo(x2,y2);}
  else if(_brdShType==='arrow') {
    _brdX.moveTo(x1,y1);_brdX.lineTo(x2,y2);
    var ang=Math.atan2(y2-y1,x2-x1),hs=12+_brdSz;
    _brdX.moveTo(x2,y2);_brdX.lineTo(x2-hs*Math.cos(ang-Math.PI/6),y2-hs*Math.sin(ang-Math.PI/6));
    _brdX.moveTo(x2,y2);_brdX.lineTo(x2-hs*Math.cos(ang+Math.PI/6),y2-hs*Math.sin(ang+Math.PI/6));
  }
  if(_brdShFill&&_brdShType!=='line'&&_brdShType!=='arrow'){
    _brdX.globalAlpha=0.18;_brdX.fillStyle=_brdColor;_brdX.fill();_brdX.globalAlpha=1;
  }
  _brdX.stroke();
}
...
function _brdObjBbox(obj){
  if(obj.type==='stroke'||obj.type==='erase'){
    var pts=obj.pts,minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
    for(var i=0;i<pts.length;i++){
      if(pts[i].x<minX)minX=pts[i].x;if(pts[i].x>maxX)maxX=pts[i].x;
      if(pts[i].y<minY)minY=pts[i].y;if(pts[i].y>maxY)maxY=pts[i].y;
    }
    var pad=(obj.size||4)/2+4;
    return{x:minX-pad,y:minY-pad,w:maxX-minX+pad*2,h:maxY-minY+pad*2};
  }
  if(obj.type==='shape'){
    var x=Math.min(obj.x1,obj.x2),y=Math.min(obj.y1,obj.y2);
    var w=Math.abs(obj.x2-obj.x1),h=Math.abs(obj.y2-obj.y1);
    var pad=(obj.size||2)/2+4;
    return{x:x-pad,y:y-pad,w:w+pad*2,h:h+pad*2};
  }
  if(obj.type==='text'){
    if(_brdX){
      _brdX.save();
      _brdX.font=(obj.italic?'italic ':'normal ')+(obj.bold?'700':'400')+' '+obj.textSize+'px "Plus Jakarta Sans",sans-serif';
      var lines=obj.content.split('\n'),maxW=0;
      for(var i=0;i<lines.length;i++){var m=_brdX.measureText(lines[i]);if(m.width>maxW)maxW=m.width;}
      _brdX.restore();
      var lh=obj.textSize*1.45,totalH=obj.textSize+lh*(lines.length-1)+obj.textSize*0.4;
      var ox=obj.align==='center'?obj.x-maxW/2:obj.align==='right'?obj.x-maxW:obj.x;
      return{x:ox-8,y:obj.y-obj.textSize-4,w:maxW+16,h:totalH+8};
    }
...
function _brdPtSegDist(px,py,ax,ay,bx,by){
...
function _brdHitTest(px,py){
  // Retourne l'ID du premier objet sous le point (plus récent en premier)
  for(var i=_brdObjects.length-1;i>=0;i--){
    var obj=_brdObjects[i];
    if(obj.type==='erase')continue;
    var bb=_brdObjBbox(obj);
    if(px<bb.x||px>bb.x+bb.w||py<bb.y||py>bb.y+bb.h)continue;
    if(obj.type==='stroke'){
      var pts=obj.pts,thresh=Math.max((obj.size||4)/2+8,10);
      for(var j=1;j<pts.length;j++){
        if(_brdPtSegDist(px,py,pts[j-1].x,pts[j-1].y,pts[j].x,pts[j].y)<thresh)return obj.id;
      }
    }else{return obj.id;}
  }
  return null;
...
function _brdObjTranslated(obj,dx,dy){
...
function _brdRenderObj(obj,ctx){
  ctx.save();
  if(obj.type==='stroke'){
    ...
    ctx.strokeStyle='rgba(0,0,0,1)';ctx.lineWidth=obj.size;
    ctx.lineCap='round';ctx.lineJoin='round';ctx.stroke();
    ...
    if(obj.shType==='rect'){
      if(obj.fill){ctx.fillStyle=obj.color;ctx.globalAlpha=0.18;ctx.fillRect(Math.min(x1,x2),Math.min(y1,y2),Math.abs(x2-x1),Math.abs(y2-y1));ctx.globalAlpha=1;}
      ctx.beginPath();ctx.strokeRect(Math.min(x1,x2),Math.min(y1,y2),Math.abs(x2-x1),Math.abs(y2-y1));
    ...
    }else if(obj.shType==='arrow'){
      var ang=Math.atan2(y2-y1,x2-x1),aLen=Math.min(20,Math.hypot(x2-x1,y2-y1)*0.4);
      ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();
      ctx.beginPath();ctx.moveTo(x2,y2);ctx.lineTo(x2-aLen*Math.cos(ang-0.4),y2-aLen*Math.sin(ang-0.4));ctx.moveTo(x2,y2);ctx.lineTo(x2-aLen*Math.cos(ang+0.4),y2-aLen*Math.sin(ang+0.4));ctx.stroke();
    }
  }else if(obj.type==='text'){
    ctx.globalCompositeOperation='source-over';ctx.globalAlpha=1;
    ctx.font=(obj.italic?'italic ':'normal ')+(obj.bold?'700':'400')+' '+obj.textSize+'px "Plus Jakarta Sans",sans-serif';
...
function _brdRenderAll(){
  ...
  _brdX.clearRect(0,0,_brdC.width,_brdC.height);
  _brdX.restore();
  for(var i=0;i<_brdObjects.length;i++)_brdRenderObj(_brdObjects[i],_brdX);
}

// ── Object Selection UI ───────────────────────────────────────────────────────
function _brdActivateObjSel(ids){
  if(!ids||!ids.length)return;
  _brdCleanObjSel();
  _brdObjSel.active=true;_brdObjSel.ids=ids.slice();
...
function _brdShowObjSelOverlay(){
  ...
  var el=document.createElement('div');
  el.id='_brdObjSelEl';
  el.style.cssText='position:absolute;left:'+(bb.x-PAD)+'px;top:'+(bb.y-PAD)+'px;'
    +'width:'+(bb.w+PAD*2)+'px;height:'+(bb.h+PAD*2)+'px;'
    +'box-sizing:border-box;pointer-events:none;z-index:60;'
  ...
  var ic='rgba(255,255,255,.85)';
  function ibtn(fn,svg,title,col){
    return '<button onclick="'+fn+'" style="background:none;border:none;cursor:pointer;'
  ...
  bar.style.cssText='position:fixed;z-index:10200;background:rgba(20,24,38,.96);border-radius:12px;'
    ...
    +'box-shadow:0 4px 20px rgba(0,0,0,.5);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);';
...
function _brdUpdateObjSelBarPos(){
  var bar=_brdObjSel.bar;if(!bar)return;
  ...
  var top=r.bottom+8;
  if(top+40>window.innerHeight)top=Math.max(8,r.top-48);
...
function _brdObjSelDelete(){
  ...
  _boardSaveHist();
  var ids=_brdObjSel.ids.slice();
  _brdObjects=_brdObjects.filter(function(o){return ids.indexOf(o.id)===-1;});
  _brdRenderAll();_boardSavePage();_boardSaveHist();_brdCleanObjSel();
  if(_brdRoomId&&_brdCanEdit)_brdEmitOp({type:'objdelete',ids:ids});
  haptic(2);
}

function _brdObjSelDuplicate(){
  if(!_brdObjSel.active||!_brdObjSel.ids.length)return;
  _boardSaveHist();
  var newIds=[];
  _brdObjSel.ids.forEach(function(id){
    var obj=_brdObjects.find(function(o){return o.id===id;});if(!obj)return;
    var copy=JSON.parse(JSON.stringify(obj));copy.id=_brdObjId();
    if(copy.type==='stroke'||copy.type==='erase'){copy.pts=copy.pts.map(function(p){return{x:p.x+20,y:p.y+20};});}
    else if(copy.type==='shape'){copy.x1+=20;copy.y1+=20;copy.x2+=20;copy.y2+=20;}
    else if(copy.type==='text'){copy.x+=20;copy.y+=20;}
    _brdObjects.push(copy);newIds.push(copy.id);
    if(_brdRoomId&&_brdCanEdit)_brdEmitOp({type:'objinsert',obj:copy});
  });
  _brdRenderAll();_boardSavePage();_boardSaveHist();
  _brdActivateObjSel(newIds);haptic(1);
}

function _brdEditTextObj(obj){
  // Retire l'objet, ouvre l'éditeur préfillé
  _brdObjects=_brdObjects.filter(function(o){return o.id!==obj.id;});
  _brdRenderAll();_brdCleanObjSel();
  _brdTextSize=obj.textSize;_brdTextBold=obj.bold;_brdTextItalic=obj.italic;
  _brdTextAlign=obj.align||'left';_brdColor=obj.color;
  var cx=obj.x-(obj.align==='center'?70:obj.align==='right'?140:0);
  var cy=obj.y-obj.textSize;
  _boardInsertText(cx,cy);
  var inp=document.getElementById('_brdActiveInp');
  if(inp){inp.value=obj.content;inp.dispatchEvent(new Event('input'));inp.setSelectionRange(0,obj.content.length);}
}
...
function _boardExport(){
  ...
  // Fond blanc explicite pour éviter fond transparent/noir dans le PNG exporté
...
function _boardInsertText(cx,cy){
  ...
  var commit=function(){
    ...
    // Émettre l'op texte
    if(_brdRoomId&&_brdCanEdit){
      _brdEmitOp({type:'text',id:_txtId,x:tx,y:cy+_brdTextSize,content:txt,
        textSize:_brdTextSize,bold:_brdTextBold,italic:_brdTextItalic,
        align:_brdTextAlign,color:_brdColor});
    }
  };
...
function _brdUserColor(uid){
  // Couleur déterministe basée sur un hash du userId — stable quelle que soit l'ordre d'arrivée
...
function _brdOnRemoteStrokeEnd(d){
  ...
  if(el)setTimeout(function(){
    if(_brdRemoteStrokes[d.userId])return; // nouveau trait entre-temps
    el.style.opacity='0';
    setTimeout(function(){if(el.parentNode&&!_brdRemoteStrokes[d.userId])el.parentNode.removeChild(el);},350);
  },2000);
}
...
function _brdApplyRemoteOp(op){
  ...
  if(op.type==='stroke'||op.type==='erase'){
    ...
    // Enregistrer dans le modèle objet si ID fourni et pas déjà présent
    if(op.id&&!_brdObjects.find(function(o){return o.id===op.id;})){
      if(op.type==='erase')_brdObjects.push({id:op.id,type:'erase',pts:pts,size:op.size});
      else _brdObjects.push({id:op.id,type:'stroke',tool:op.tool,pts:pts,color:op.color,size:op.size});
    }
    _brdX.save();
    ...
    if(op.type==='erase'){
      _brdX.globalCompositeOperation='destination-out';
      _brdX.strokeStyle='rgba(0,0,0,1)';_brdX.lineWidth=op.size;
    }else{
      _brdX.globalCompositeOperation='source-over';
      _brdX.strokeStyle=op.color;
      _brdX.globalAlpha=op.tool==='marker'?0.38:1;
      _brdX.lineWidth=op.tool==='marker'?op.size*2.5:op.size;
    }
    _brdX.lineCap='round';_brdX.lineJoin='round';_brdX.stroke();
    _brdX.globalAlpha=1;_brdX.globalCompositeOperation='source-over';
    _brdX.restore();
    ...
    if(op.id&&!_brdObjects.find(function(o){return o.id===op.id;})){
      _brdObjects.push({id:op.id,type:'shape',shType:op.shType,x1:op.x1,y1:op.y1,x2:op.x2,y2:op.y2,color:op.color,size:op.size,fill:op.fill});
    }
    ...
    _brdX.save();_boardDrawShape(op.x1,op.y1,op.x2,op.y2);_brdX.restore();
    _brdColor=oc;_brdSz=os;_brdShType=ot;_brdShFill=of;
  }else if(op.type==='text'){
    if(op.id&&!_brdObjects.find(function(o){return o.id===op.id;})){
      _brdObjects.push({id:op.id,type:'text',content:op.content,x:op.x,y:op.y,
        textSize:op.textSize,bold:op.bold,italic:op.italic,align:op.align,color:op.color});
    }
    _brdX.save();
    ...
    _brdX.fillStyle=op.color;_brdX.textAlign=op.align||'left';
    ...
    if(typeof op.bgType==='string'){
      _brdBgType=op.bgType;_boardDrawBg();
      var _bgb=g('_brdBgBtn');if(_bgb)_bgb.style.opacity=_brdBgType==='grid'?'1':'0.45';
    }
  }else if(op.type==='objmove'&&op.ids){
    op.ids.forEach(function(id){
      var idx=_brdObjects.findIndex(function(o){return o.id===id;});
      if(idx!==-1)_brdObjects[idx]=_brdObjTranslated(_brdObjects[idx],op.dx,op.dy);
    });
...
// Recevoir un changement de permission
function _brdOnPerm(data){
  if(!user)return;
  if(data.userId===user.id){
    _brdCanEdit=data.canEdit;
    _brdUpdatePermUI();
    if(data.canEdit)toast('✏️ Le prof vous autorise à dessiner','');
    else toast('Accès dessin retiré','');
  }
  // Mettre à jour la liste participants
  var p=_brdParticipants.find(function(x){return x.id===data.userId;});
  if(p)p.canEdit=data.canEdit;
  _brdRenderPermPanel();
}

// Nouveau participant rejoint la board room
function _brdOnParticipantJoined(data){
  if(!_brdParticipants.find(function(x){return x.id===data.userId;})){
    _brdParticipants.push({id:data.userId,name:data.userName,canEdit:false});
  }
  _brdRenderPermPanel();
}
...
// Mettre à jour l'UI du toolbar (cadenas visible si lecture seule, toolbar dimmed)
function _brdUpdatePermUI(){
  var lock=g('_brdLockBadge');
  if(lock)lock.style.display=(_brdRoomId&&!_brdCanEdit)?'flex':'none';
  _boardRenderSubbar(); // re-render pour appliquer/retirer le dimming lecture seule
}

// Rendre le panneau de gestion des permissions (prof seulement)
function _brdRenderPermPanel(){
  var panel=g('_brdPermPanel');if(!panel)return;
  if(!_isOwner||!_brdRoomId||_brdParticipants.length===0){
    panel.style.display='none';return;
  }
  panel.style.display='block';
  var others=_brdParticipants.filter(function(x){return !user||x.id!==user.id;});
  panel.innerHTML='<div style="font-size:10px;font-weight:700;color:rgba(255,255,255,.45);letter-spacing:.04em;margin-bottom:6px;">PARTICIPANTS</div>'
    +others.map(function(p){
      var can=p.canEdit;
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;gap:8px;">'
        +'<span style="font-size:12px;color:rgba(255,255,255,.8);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:90px">'+escH(p.name)+'</span>'
        +'<button onclick="_brdToggleEdit(\''+p.id+'\','+can+')" style="flex-shrink:0;padding:3px 8px;border-radius:8px;border:none;cursor:pointer;font-size:10px;font-weight:700;font-family:inherit;background:'+(can?'#FF6B2B':'rgba(255,255,255,.12)')+';color:'+(can?'#fff':'rgba(255,255,255,.6)')+';">'+(can?'Retirer':'Autoriser')+'</button>'
        +'</div>';
    }).join('');
}

// Toggle permission d'un participant
function _brdToggleEdit(userId,currentCanEdit){
  if(!_brdRoomId||typeof _socket==='undefined'||!_socket)return;
  var ev=currentCanEdit?'board_revoke':'board_grant';
  _socket.emit(ev,{roomId:_brdRoomId,userId:userId});
  haptic(1);
}

// ── Sync pages distantes (ajout / suppression / renommage) ───────────────────
...
function _brdOnRemotePageRename(data){
  if(!_boardActive)return;
  if(typeof data.pageIdx==='number'&&data.pageIdx<_brdPages.length){
    _brdPageNames[data.pageIdx]=data.name||'';
    _boardUpdatePageTabs();
  }
}

// Émettre l'ajout / suppression / renommage de page pour les autres participants
function _brdEmitPageAdd(){
  if(_brdRoomId&&typeof _socket!=='undefined'&&_socket&&_socket.connected){
    _socket.emit('board_page_add',{roomId:_brdRoomId,pageIdx:_brdPages.length-1,name:_brdPageNames[_brdPages.length-1]||''});
  }
}
function _brdEmitPageDelete(idx){
  if(_brdRoomId&&typeof _socket!=='undefined'&&_socket&&_socket.connected){
    _socket.emit('board_page_delete',{roomId:_brdRoomId,pageIdx:idx});
  }
}
function _brdEmitPageRename(idx,name){
  if(_brdRoomId&&typeof _socket!=='undefined'&&_socket&&_socket.connected){
    _socket.emit('board_page_rename',{roomId:_brdRoomId,pageIdx:idx,name:name});
  }
}
